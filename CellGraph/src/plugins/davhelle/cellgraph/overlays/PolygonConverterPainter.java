...
public class PolygonConverterPainter extends Overlay{
	...
	private GeometryFactory geometry_factory;
	...
	public PolygonConverterPainter(SpatioTemporalGraph spatioTemporalGraph){
		...
		this.geometry_factory = new GeometryFactory();

		//Cycle through all time points
		for(int time_point=0; time_point < stGraph.size(); time_point++){
			
			System.out.print("converting frame "+time_point);
			...
			for(Node n: stGraph.getFrame(time_point).vertexSet()){
				...
				//do not analyze cells with less than 3 neighbors 
				//i.e. minimal polygon is a triangle
				if(n.getNeighbors().size() < 3)
					continue;
				
				Polygon node_tile = getPolygonalTile(n);
				
				if(node_tile != null)
					polyMap.put(n, node_tile);
				else
					wrong_polygons++;
				
			}
			System.out.println(String.format("(%d conflicting polygons)",wrong_polygons));
		}
	}
	...
	private Polygon getPolygonalTile(Node n){
		...
		if(ringCoords.size() >= 4)
			nPoly = geometry_factory.createPolygon(ringCoords.toArray(new Coordinate[idx]));
