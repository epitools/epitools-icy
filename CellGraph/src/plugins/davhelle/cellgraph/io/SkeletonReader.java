...
public class SkeletonReader implements PolygonReader{
	...
	private final boolean REDO_SKELETON;
	private SegmentationProgram SEG_TOOL;
	 ...
	 */
	public SkeletonReader(boolean redo_skeleton, SegmentationProgram tool){

		lineFactory = new GeometryFactory();
		this.REDO_SKELETON = redo_skeleton;
		this.SEG_TOOL = tool;
	...
	private BufferedImage reskeletonize(String file_name) {
		...
		//Image Type checks
		//System.out.println(original_image.getBitDepth());
		//System.out.println(original_image.getType());
		
		//special 8-bit RGB files generated by MATLAB
		//when converting a label image into a skeleton
		//i.e. removes double labels
		if(SEG_TOOL == SegmentationProgram.MatlabLabelOutlines){
			ij.IJ.run(original_image, "8-bit","");
			ij.IJ.run(original_image, "Invert","");
			ij.IJ.run(original_image, "Skeletonize", "");
			ij.IJ.run(original_image, "Invert","");
			
		}
		
		if(SEG_TOOL == SegmentationProgram.SeedWater){
	...
	public ArrayList<Polygon> extractPolygons(String file_name){
		...
		//redo skeletonization using imageJ in case of particular 
		//input programs as SeedWaterSegmenter or custom Matlab outputs
		if(REDO_SKELETON)
